...
function getRange(number) {
  return Math.min(Math.max(Math.floor(number / 7), 2), 5);
}
...
export default {
  ...
  activate(state) {
    ...
    this.bassSequence.gain.gain.value = 0.45;
  ...
  handleKey(e) {
    ...
    if (number) {
      ...
      if (Math.random() > 0.75) {
        this.sequence.push('- q');
      }

      if (Math.random() < 0.2) {
        this.sequence.push(`${getNote(number + 1)}${range} s`, '- s');
      }
    }
  },
